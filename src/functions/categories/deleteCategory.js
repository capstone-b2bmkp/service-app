import models from '../../models';
import { checkAuth } from '../../helpers/auth/checkAuth';
import { checkAdmin } from '../../helpers/auth/checkAdmin';
import { succesfullResponse, errorResponse } from '../../utils/response_util';

export const handler = async (event) => {
  console.log('estoy en deleteCategory handler');
  try {
    // esto exige que el usuario este logueado
    const checkValidTokenResult = checkAuth(event);
    if (checkValidTokenResult.statusCode !== 200) {
      return errorResponse({ error: 'Token invalido, no se puede crear la categoria' }, 500);
    }

    // esto revisa que el token enviado corresponda a un usuario admin
    const checkAdminResult = checkAdmin(checkValidTokenResult);
    if (checkAdminResult.statusCode !== 200) {
      return errorResponse({ error: 'No autorizado' }, 401);
    }
    const { id } = event.pathParameters;
    const body = JSON.parse(event.body);
    const { backUpCategoryId } = body.backUpCategoryId;
    console.log(backUpCategoryId);
    const category = await models.Category.findOne({ where: { id: id } });
    if (!category) {
      return errorResponse({ error: `Category with id ${id} not found` }, 400);
    }
    const products = await models.Product.findAll({ where: { categoryId: backUpCategoryId } });
    products.forEach(async product => {
      const updates = {};
      updates.categoryId = backUpCategoryId;
      await models.Product.update(updates, { where: { id: product.id } });
    });
    // change companies
    await models.Category.destroy({ where: { id: id } });
    return succesfullResponse({ message: 'Category deleted successfully' }, 200);
  } catch (error) {
    return errorResponse({ error: 'No se pudo eliminar la categoria' }, 500);
  }
};

// https://zellwk.com/blog/async-await-in-loops/
